...
Database = 'SwissProt'
Missed_cleavages = 1
Anottate_CSV_File = False #Criar ou não tabela de resultados
Console_mode = True      #Mostrar ou não o navegador trabalhando

Peaklist_Path = 'C:/Users/Rodrigo-PCG/OneDrive/LGPP/MASCOT_AUTOMATION/MascotAuto/Pyscot-ATT/peaklists'
Results_Path = 'C:/Users/Rodrigo-PCG/OneDrive/LGPP/MASCOT_AUTOMATION/MascotAuto/Pyscot-ATT/results'
...
if Anottate_CSV_File == True:
    ...
    csv_file.close()
## END
##Login on proteomicsresource

def pleaseLogin():
    print(Fore.YELLOW + Style.BRIGHT + 'Enter Database User: ')
    DB_User = input()
    driver.find_element_by_id('username').send_keys(str(DB_User))
    print(Fore.YELLOW + Style.BRIGHT + 'Enter Database Password: ')
    DB_Password = input()
    driver.find_element_by_name('password').send_keys(str(DB_Password))
##
for peaklist in Peaklists:
    for pepComb in Peptide_combinations:

        driver.get('https://proteomicsresource.washington.edu/mascot/cgi/search_form.pl?FORMVER=2&SEARCH=PMF')
        try:
            if driver.find_element_by_xpath('//*[@id="plainContent"]/table/tbody/tr/td/h2').is_displayed():
                pleaseLogin()
                print(Fore.YELLOW + 'Logging in DB...')
                driver.find_element_by_xpath('//*[@id="mascot_login"]/table/tbody/tr[3]/td/input').click()
                print(Fore.LIGHTGREEN_EX + 'Logged sucessful.')
        except:
        ...
        data_Base_Select.deselect_by_value('Gossypium_hirsutum')
        ...
        driver.find_element_by_xpath('//*[@id="plainContent"]/form/table/tbody/tr[16]/td[2]/b/input').submit()
        print(Fore.CYAN + Style.DIM + 'Searching...')

        try:
            waiter = wdWait(driver, 50).until(
                url_contains('https://proteomicsresource.washington.edu/mascot/cgi/master_results.pl'))
            ...
            if topScore > SignificScore:
                ...
                if Anottate_CSV_File == True:
                    ...
                    content.append('"%s";"%s";"%s";"%s";"%s";"%s";"%s";"%s";"%s";"%s"\n' % (
                        str(peaklist), Taxonomy_clean, Database, pepComb[1] + " " + pepComb[0], Mascot_ID, Protein_name,
                        topScore,
